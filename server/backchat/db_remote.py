...
def get_conn() -> sqlite3.Connection:
    ...
    if _conn is None:
        ...
        # Migrate users: add premium columns
        ...
        # Migrate users: premium profile columns
        for _pcol in [
...
def set_premium(username: str, enabled: bool, until: int = 0) -> bool:
    """Выдать/снять премиум. until — unix-timestamp окончания (0 = бессрочно)."""
    with _lock:
        if not user_exists(username):
            return False
        get_conn().execute(
            "UPDATE users SET is_premium = ?, premium_until = ? WHERE username = ?",
            (1 if enabled else 0, until, username),
        )
        get_conn().commit()
        return True
...
def get_reactions(msg_id: str) -> list:
    """Возвращает спис��к {emoji, username, count} для сообщения."""
    with _lock:
        rows = get_conn().execute(
            "SELECT emoji, username FROM message_reactions WHERE msg_id = ? ORDER BY created_at",
            (msg_id,)
        ).fetchall()
    result: dict[str, list] = {}
    for r in rows:
        result.setdefault(r["emoji"], []).append(r["username"])
    return [{"emoji": e, "users": ulist, "count": len(ulist)} for e, ulist in result.items()]


def add_reaction(msg_id: str, username: str, emoji: str) -> bool:
    """Добавляет реакцию. True = добавлена, False = уже была (идемпотентно)."""
    with _lock:
        try:
            get_conn().execute(
...
def remove_reaction(msg_id: str, username: str, emoji: str) -> bool:
    """Удаляет реакцию. True = удалена, False = не было."""
    with _lock:
        cur = get_conn().execute(
...
def count_user_reactions(msg_id: str, username: str) -> int:
    """Сколько разных эмодзи поставил юзер на это сообщение."""
    with _lock:
        row = get_conn().execute(
            "SELECT COUNT(*) FROM message_reactions WHERE msg_id=? AND username=?",
            (msg_id, username)
        ).fetchone()
    return row[0] if row else 0


def user_has_reaction(msg_id: str, username: str, emoji: str) -> bool:
    with _lock:
        row = get_conn().execute(
...
def get_pending_messages(to_username: str) -> List[sqlite3.Row]:
    ...
    with _lock:
        ...
        return get_conn().execute(
            ...
            " FROM messages WHERE to_username = ? AND delivered = 0 AND delivery_attempts < 5 ORDER BY sent_at ASC",
...
def increment_delivery_attempts(msg_id: str) -> None:

    with _lock:

        get_conn().execute(

            "UPDATE messages SET delivery_attempts = delivery_attempts + 1 WHERE msg_id = ?",
...
def are_contacts(user_a: str, user_b: str) -> bool:
    """Return True if user_a and user_b are mutual contacts."""
    with _lock:
        row = get_conn().execute(
...
def cleanup_expired_logins() -> None:
    """Remove pending login requests that have expired."""
    cutoff = int(time.time())
    with _lock:
        get_conn().execute(
            "DELETE FROM pending_logins WHERE expires_at > 0 AND expires_at < ? "
            "AND phase NOT IN ('success', 'pending')",
            (cutoff,)
        )
        get_conn().commit()
...
def get_channel_by_tag(tag: str) -> Optional[sqlite3.Row]:

    with _lock:

        return get_conn().execute(

            "SELECT * FROM channels WHERE tag = ?", (tag,)
...
def delete_channel(channel_id: str) -> None:
    ...
    with _lock:
        ...
        try:
            get_conn().execute("DELETE FROM channel_keys WHERE channel_id = ?", (channel_id,))
        except Exception:
            pass
        try:
            get_conn().execute("DELETE FROM channel_join_requests WHERE channel_id = ?", (channel_id,))
        except Exception:
            pass
...
def get_user_total_file_size(username: str) -> int:
    with _lock:
        row = get_conn().execute(
            "SELECT COALESCE(SUM(file_size), 0) FROM pending_files WHERE from_username = ?",
            (username,),
        ).fetchone()
        return row[0] if row else 0
...
def remove_device(username: str, device_id: str) -> None:
    """Remove a device from the registry without blocking it (can re-register later)."""
    with _lock:
        get_conn().execute(
            "DELETE FROM device_registry WHERE username=? AND device_id=?",
            (username, device_id)
        )
        get_conn().commit()


def get_blocked_devices(username: str) -> list:
    """Return all blocked devices for a user."""
    with _lock:
        return list(get_conn().execute(
            "SELECT device_id, device_name, registered_at FROM device_registry"
            " WHERE username=? AND blocked=1",
            (username,)
        ).fetchall())


def unblock_device(username: str, device_id: str) -> None:
    """Remove the block on a device (deletes the blocked record)."""
    with _lock:
        get_conn().execute(
            "DELETE FROM device_registry WHERE username=? AND device_id=? AND blocked=1",
            (username, device_id)
        )
        get_conn().commit()
...
def force_set_creator(username: str, device_id: str) -> bool:
    """Принудительно назначить устройство создателем (игнорирует защиту creator)."""
    with _lock:
        conn = get_conn()
        row = conn.execute(
            "SELECT 1 FROM device_registry WHERE username=? AND device_id=? AND blocked=0",
            (username, device_id)
        ).fetchone()
        if not row:
            return False
        conn.execute(
            "UPDATE device_registry SET trust_type='creator', trust_level=0, permissions='{}' "
...
def transfer_creator_and_remove(username: str, old_creator_device_id: str) -> str:
    """
    Переносит роль creator на лучшее оставшееся устройство, затем удаляет старого creator.
    Возвращает device_id нового creator или '' если других устройств нет.
    """
    with _lock:
        conn = get_conn()
        # Получаем все остальные активные устройства
        rows = conn.execute(
            "SELECT device_id, trust_level, permissions FROM device_registry "
            "WHERE username=? AND device_id != ? AND blocked=0",
            (username, old_creator_device_id)
        ).fetchall()

        new_creator_id = ''
        if rows:
            # Выбираем устройство с наибольшим trust_level, при равенстве — с большим кол-вом прав
            best = max(rows, key=lambda r: (
                r["trust_level"],
                len(json.loads(r["permissions"] or '{}'))
            ))
            new_creator_id = best["device_id"]
            conn.execute(
                "UPDATE device_registry SET trust_type='creator', trust_level=0, permissions='{}' "
                "WHERE username=? AND device_id=?",
                (username, new_creator_id)
            )

        # Удаляем старого creator
        conn.execute(
            "DELETE FROM device_registry WHERE username=? AND device_id=?",
            (username, old_creator_device_id)
        )
...
def init_wipe_pending_table() -> None:
    """Create table to track devices that haven't acked a wipe yet."""
    with _lock:
        get_conn().executescript("""
        CREATE TABLE IF NOT EXISTS wipe_pending (
            device_id   TEXT NOT NULL,
            username    TEXT NOT NULL,
            created_at  INTEGER NOT NULL DEFAULT 0,
...
def set_all_devices_wipe_pending() -> None:
    """Mark all registered devices as pending wipe."""
    with _lock:
        conn = get_conn()
        try:
            conn.execute("DELETE FROM wipe_pending")
            conn.execute("""
                INSERT OR IGNORE INTO wipe_pending (device_id, username, created_at)
                SELECT device_id, username, strftime('%s','now') FROM device_registry
            """)
            conn.commit()
        except Exception:
            pass

def ack_wipe(device_id: str) -> None:
    with _lock:
        get_conn().execute("DELETE FROM wipe_pending WHERE device_id=?", (device_id,))
        get_conn().commit()

def has_pending_wipes() -> bool:
    row = get_conn().execute("SELECT COUNT(*) FROM wipe_pending").fetchone()
    return (row[0] if row else 0) > 0

def is_wipe_pending_for_device(device_id: str) -> bool:
    row = get_conn().execute(
        "SELECT 1 FROM wipe_pending WHERE device_id=?", (device_id,)).fetchone()
    return row is not None

def wipe_all_data() -> None:
    """Delete all user data from every table. Server structure (schema) stays intact.
    device_registry is intentionally kept so offline devices can reconnect and receive wipe_all.
    wipe_pending is intentionally kept to track who hasn't acked yet.
    """
    tables = [
        "messages", "message_receipts", "offline_events", "contacts",
        "channels", "channel_keys", "channel_join_requests", "channel_members",
        "channel_messages", "pending_files", "file_requests",
        "support_messages", "pending_logins", "users",
        # device_registry — NOT deleted: offline devices need it to reconnect and get wipe_all
        # wipe_pending    — NOT deleted: must survive to track unacked devices
    ]
    with _lock:
        conn = get_conn()
        for t in tables:
...
def delete_user(username: str) -> List[str]:
    """Delete all data for a user. Returns list of their contacts (to notify them)."""
    with _lock:
        conn = get_conn()
        # Contacts table uses user_a/user_b columns
        rows = conn.execute(
            "SELECT user_a, user_b FROM contacts WHERE user_a=? OR user_b=?",
            (username, username)).fetchall()
        contacts = [
            r["user_b"] if r["user_a"] == username else r["user_a"]
            for r in rows
        ]
        # Remove this user from all their contacts' lists too
        conn.execute("DELETE FROM contacts WHERE user_a=? OR user_b=?", (username, username))
        # Delete messages
        conn.execute("DELETE FROM messages WHERE from_username=? OR to_username=?", (username, username))
        try:
            conn.execute("DELETE FROM message_receipts WHERE from_username=? OR to_username=?", (username, username))
        except Exception:
            pass
        conn.execute("DELETE FROM offline_events WHERE to_username=?", (username,))
        try:
            conn.execute("DELETE FROM pending_files WHERE from_username=? OR to_username=?", (username, username))
        except Exception:
            pass
        conn.execute("DELETE FROM device_registry WHERE username=?", (username,))
        conn.execute("DELETE FROM pending_logins WHERE username=?", (username,))
        try:
            conn.execute("DELETE FROM wipe_pending WHERE username=?", (username,))
        except Exception:
            pass
        try:
            conn.execute("DELETE FROM support_messages WHERE from_username=? OR to_username=?", (username, username))
        except Exception:
            pass
...
def get_all_usernames() -> List[str]:
    rows = get_conn().execute("SELECT username FROM users").fetchall()
...
def get_server_stats() -> dict:
    ...
    return {
        ...
        "channel_messages": count("channel_messages"),
        "pending_files":    count("pending_files"),
        "devices":          count("device_registry"),
        "pending_logins":   count("pending_logins"),
        "support_messages": count("support_messages"),
    }
